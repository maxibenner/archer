...
async function init3D() {
    ...
    // Hemisphere Light
    var ambientLight = new THREE.AmbientLight(0xffffff, .8/*.95*/)
    ...
    loader.load(glbPath,
        function (gltf) {
            ...
            camera.position.set(0, size.y / 2, 4)
            ...
            // Set camera position for cart item
...
/*____________________ Modify Background ______________________*/

// Change background height
prodHeight.oninput = function () {
    set.position.y = ((this.value)*(-1)) / 200
    bgPosY = set.position.y
...
colorPicker.oninput = function () {
    ...
    // Format color value
    let hexColor = this.value.replace(/#/g, '0x')

    // Set cart
    backgroundColor = hexColor
    console.log(hexColor)

    // Change threejs background
    set.traverse(n => {
        if (n.isMesh) {
            n.material.color.setHex(hexColor)
        }
    })

    // Set click box color
    colorClickBox.style.background = this.value
...
// Transparency Switch
transparencySwitch.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('panel-toggle')) {
        if (e.target.classList.contains('panel-toggle_active')) {
            ...
            transparencyEffector.forEach((e)=>{
                e.style.opacity = 1
...
//____________________ Move Camera ______________________//
camMes.oninput = function () {
    camera.position.y = modelHeight.y / 2 + (this.value/200)
    //camera.lookAt(camera.position.x, modelHeight.y / 2, 0)
    cameraPositionRotationBlender()

    // Refresh render
    renderer.render(scene, camera)
}
camLat.oninput = function () {
    camera.position.x = this.value / 300
    cameraPositionRotationBlender()

    // Refresh render
    renderer.render(scene, camera)
}
camZoom.oninput = function () {
    camera.position.z = 4 - this.value / 10
    cameraPositionRotationBlender()
...
camTilt.oninput = function () {
    camera.rotation.x = (this.value / 400)*-1
    cameraPositionRotationBlender()
...
// Converts camera position to Blender coordinates
function cameraPositionRotationBlender() {
    cameraPosX = camera.position.x
    cameraPosY = camera.position.z * -1
    cameraPosZ = camera.position.y
    cameraRotationX = 90 + THREE.MathUtils.radToDeg(camera.rotation.x)
...
// Float Switch
floatSwitch.addEventListener('click', (e) => {
